...
export const editRecord = async ({
  ...
  return extendRequest('http://localhost:8080/system/dept/update', {
    method: 'post',
    data: { id, ...values },
  })
    .then(function (response) {
...
export const addRecord = async ({ values }: { values: FormValues }) => {
  return extendRequest('http://localhost:8080/system/dept/add', {
    method: 'post',
    data: values,
  })
    .then(function (response) {
...
export const delRecord = async ({ id }: { id: string }) => {
  return extendRequest(`http://localhost:8080/system/dept/delete/${id}`, {
    method: 'delete',
  })
    .then(function (response) {
      return true;
