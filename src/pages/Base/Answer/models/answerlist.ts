...
export interface answerListState {
  pageNum?: number;
  pageSize?: number;
  total?: number;
  list?: AnswerType[];
...
export interface AnswerListModelType {
  namespace: 'answers';
  state: answerListState;
  reducers: {
    getList: Reducer<answerListState>;
  };
  effects: {
    fetchList: Effect;
    fetchUpdate: Effect;
...
const AnswerListModel: AnswerListModelType = {
  namespace: 'answers',
  state: {},
  reducers: {
  ...
  subscriptions: {
    setup({ dispatch, history }) {
      return history.listen((location, action) => {
        if (location.pathname === '/base/answer') {
          dispatch({
            type: 'fetchList',
